...
public class TestDeadlockReplanning {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		Pose goalTwoRobot2 = new Pose(5.0,2.0,Math.PI);
		...
		tec.addMissions(Missions.getMission(2, 0), Missions.getMission(1, 0));
		tec.computeCriticalSections();
		tec.startTrackingAddedMissions();

		Thread.sleep(13000);
		
		rsp.setStart(goalRobot2);
		rsp.setGoals(goalTwoRobot2);
		//One way of avoiding deadlock: plan path with current pose of other robot as obstacle
		//rsp.addObstacles(new Geometry[] {tec.getObstacleFromCurrentPose(1)} );
		rsp.plan();
		Missions.putMission(new Mission(2,rsp.getPath()));
